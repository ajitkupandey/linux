...
struct acp_dev_data {
	...
	struct snd_dma_buffer temp_buf;
...
struct acp_stream *find_stream(struct acp_dev_data *adata, unsigned int val)
{
	unsigned int i = 0;
					...
					int dai_id, int direction)
{
	...
	if (direction == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		case I2S_BT_INSTANCE:
			high = acp_readl(adata->acp3x_base +
				       ACP_BT_TX_LINEARPOSITIONCNTR_HIGH);
			low = acp_readl(adata->acp3x_base +
