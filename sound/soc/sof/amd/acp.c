...
static int config_dma_channel(struct acp_dev_data *adata, unsigned int ch,
			      unsigned short idx, unsigned short dscr_count)
{
	struct snd_sof_dev *sdev = adata->dev;
	unsigned int val, status;
	int ret;

	snd_sof_dsp_write(sdev, ACP_DSP_BAR, ACP_DMA_CNTL_0 + ch * sizeof(u32), 0x11);
	...
	snd_sof_dsp_write(sdev, ACP_DSP_BAR, ACP_DMA_CNTL_0 + ch * sizeof(u32),
			  ACP_DMA_CH_RUN | ACP_DMA_CH_IOC_ENABLE);

	return ret;
}

static int acpbus_dma_start(struct acp_dev_data *adata, unsigned int ch,
			    unsigned short *dscr_count, struct dma_descriptor *dscr_info)
{
	struct snd_sof_dev *sdev = adata->dev;
	int ret = 0;
	u16 dscr, count;

	if (dscr_count && dscr_info) {
		count = *dscr_count;
		for (dscr = 0; dscr < count; dscr++)
			configure_dma_descriptor(adata, dscr, dscr_info++);

		ret = config_dma_channel(adata, ch, 0, count);
		if (ret < 0)
			  ...
			  unsigned int dest_addr, int dsp_data_size)
{
	...
	ret = acpbus_dma_start(adata, 0, (unsigned short *)&desc_count, adata->dscr_info);
	if (ret) {
		dev_err(sdev->dev, "acpbus_dma_start failed\n");
		return ret;
	}

	/* Clear descriptor array once registers configured */
	for (index = 0; index < desc_count && index < ACP_MAX_DESC; index++)
			      ...
			      unsigned int image_length)
{
	...
	if (val & 0x01) {
		snd_sof_dsp_write(sdev, ACP_DSP_BAR, ACP_SHA_DMA_CMD, 0x02);
		ret = snd_sof_dsp_read_poll_timeout(sdev, ACP_DSP_BAR, ACP_SHA_DMA_CMD_STS,
						    val, val & 0x02, ACP_REG_POLL_INTERVAL,
						    ACP_REG_POLL_TIMEOUT_US);
		if (ret < 0) {
	...
	snd_sof_dsp_write(sdev, ACP_DSP_BAR, ACP_SHA_DMA_CMD, 0x05);
...
int acp_dma_status(struct acp_dev_data *adata, unsigned char ch)
{
	...
	if (val & ACP_DMA_CH_RUN) {
		...
		if (ret < 0)
			dev_err(sdev->dev, "DMA_CHANNEL %d status error timeout\n", ch);
	}
...
static int acp_power_on(struct snd_sof_dev *sdev)
{
	...
	if (ret < 0)
		dev_err(sdev->dev, "error timeout in ACP_PGFSM_STATUS read\n");
...
static int acp_reset(struct snd_sof_dev *sdev)
{
	...
	ret = snd_sof_dsp_read_poll_timeout(sdev, ACP_DSP_BAR, ACP_SOFT_RESET, val,
					    ...
					    ACP_REG_POLL_INTERVAL, ACP_REG_POLL_TIMEOUT_US);
	if (ret < 0) {
		dev_err(sdev->dev, "error timeout asserting reset\n");
		return ret;
	}
	...
	ret = snd_sof_dsp_read_poll_timeout(sdev, ACP_DSP_BAR, ACP_SOFT_RESET, val, !val,
					    ACP_REG_POLL_INTERVAL, ACP_REG_POLL_TIMEOUT_US);
	if (ret < 0)
		dev_err(sdev->dev, "error timeout in releasing reset\n");
