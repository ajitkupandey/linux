...
static struct resource renoir_res[] = {
	{
		.start = 0,
...
static int acp_pci_rn_probe(struct pci_dev *pci, const struct pci_device_id *pci_id)
{
	 ...
	 * for some machine. Register platform device for I2S use case without SOF if machine flag
	 * is set to FLAG_SOF_ONLY_DMIC.
	 */

	if (flag == FLAG_SOF_ONLY_DMIC) {
