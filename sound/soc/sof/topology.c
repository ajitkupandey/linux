			     ...
			     struct sof_ipc_dai_config *config)
{
	...
	dev_info(scomp->dev,
		 "tplg: config ACP%d channel %d rate %d\n",
	...
	ret = sof_set_dai_config(sdev, size, link, config);
	if (ret < 0)
		dev_err(scomp->dev, "error: failed to save DAI config for ACP%d\n",
			config->dai_index);
	return ret;
}
			 ...
			 struct snd_soc_tplg_link_config *cfg)
{
	...
	case SOF_DAI_AMD_DMIC:
		ret = sof_link_acp_dmic_load(scomp, index, link, cfg, hw_config + curr_conf, config);
		break;
	default:
